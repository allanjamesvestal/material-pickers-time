...
describe('TimePicker', () => {
  ...
  beforeEach(() => {
    ...
    picker.cachedEls.clockMilitaryHoursLi =
      picker.cachedEls.clockMilitaryHours.childNodes
  })
  ...
  describe('#bindInput', () => {
    test('should assign passed options merged with defaultOptions to inputEl parameter', () => {
    ...
    test('should set focus event listener to inputEl parameter', () => {
      ...
      expect(addEventListenerSpy.calledWith('focus', sinon.match.func)).toBe(
        true,
      )
    })
  })
  ...
  describe('#openOnInput', () => {
    ...
    test('should assign passed options merged with defaultOptions to inputEl.mtpOptions property', () => {
      const inputEl = document.createElement('input')
      const options = { test: 'value' }
      const expectOptions = assign({}, picker.defaultOptions, options)
  ...
  describe('#setupTemplate', () => {
    ...
    test('should insert template in DOM if #isTemplateInDOM returns false', () => {
      isTemplateInDOMStub.onFirstCall().returns(false)
      picker.setupTemplate()

      expect(isTemplateInDOMStub.calledOnce).toBe(true)
      expect(isTemplateInDOMStub.calledBefore(insertAdjacentHTMLStub)).toBe(
        true,
      )
      expect(
        insertAdjacentHTMLStub.calledWith('beforeend', picker.template),
      ).toBe(true)
    })

    test('should not insert template in DOM if #isTemplateInDOM returns true', () => {
      ...
      expect(
        insertAdjacentHTMLStub.neverCalledWith('beforeend', picker.template),
      ).toBe(true)
    })
  })

  describe('#setEvents', () => {
    let hasSetEventsStub
    let cachedEls
    let wrapperClassListAddSpy
    ...
    beforeEach(() => {
      ...
      addEventListenerSpys.overlay = sinon.spy(
        cachedEls.overlay,
        'addEventListener',
      )
      addEventListenerSpys.buttonCancel = sinon.spy(
        cachedEls.buttonCancel,
        'addEventListener',
      )
      addEventListenerSpys.buttonFinish = sinon.spy(
        cachedEls.buttonFinish,
        'addEventListener',
      )
      addEventListenerSpys.buttonBack = sinon.spy(
        cachedEls.buttonBack,
        'addEventListener',
      )
      ...
      ;[].forEach.call(cachedEls.meridiemSpans, span => {
        addEventListenerSpys.meridiemSpans.push(
          sinon.spy(span, 'addEventListener'),
        )
      })
      ;[].forEach.call(cachedEls.clockHoursLi, listItem => {
        addEventListenerSpys.clockHoursLi.push(
          sinon.spy(listItem, 'addEventListener'),
        )
      })
      ;[].forEach.call(cachedEls.clockMinutesLi, listItem => {
        addEventListenerSpys.clockMinutesLi.push(
          sinon.spy(listItem, 'addEventListener'),
        )
      })
      ;[].forEach.call(cachedEls.clockMilitaryHoursLi, listItem => {
        addEventListenerSpys.clockMilitaryHoursLi.push(
          sinon.spy(listItem, 'addEventListener'),
        )
      })
    })
    ...
    test('should set events on cached elements if #hasSetEvents returns false', () => {
      hasSetEventsStub.onFirstCall().returns(false)
      picker.setEvents()

      expect(hasSetEventsStub.calledOnce).toBe(true)
      expect(
        addEventListenerSpys.overlay.calledWith('click', sinon.match.func),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonCancel.calledWith('click', sinon.match.func),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonFinish.calledWith('click', sinon.match.func),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonBack.calledWith('click', sinon.match.func),
      ).toBe(true)
    ...
    test('should not set events on cached elements if #hasSetEvents returns true', () => {
      hasSetEventsStub.onFirstCall().returns(true)
      picker.setEvents()

      expect(hasSetEventsStub.calledOnce).toBe(true)
      expect(
        addEventListenerSpys.overlay.neverCalledWith('click', sinon.match.func),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonCancel.neverCalledWith(
          'click',
          sinon.match.func,
        ),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonFinish.neverCalledWith(
          'click',
          sinon.match.func,
        ),
      ).toBe(true)
      expect(
        addEventListenerSpys.buttonBack.neverCalledWith(
          'click',
          sinon.match.func,
        ),
      ).toBe(true)
    ...
    test('should add .mtp-events-set to cachedEls.wrapper.classList if #hasSetEvents returns false', () => {
      hasSetEventsStub.onFirstCall().returns(false)
      picker.setEvents()
    ...
    test('should not add .mtp-events-set to cachedEls.wrapper.classList if #hasSetEvents returns true', () => {
      hasSetEventsStub.onFirstCall().returns(true)
      picker.setEvents()

      expect(hasSetEventsStub.calledOnce).toBe(true)
      expect(hasSetEventsStub.calledBefore(wrapperClassListAddSpy)).toBe(true)
      expect(wrapperClassListAddSpy.neverCalledWith('mtp-events-set')).toBe(
        true,
      )
    })
  })
  ...
  describe('#show', () => {
    let blurSpy
    let isMilitaryFormatStub
    let toggleHoursVisibleSpy
    let toggleMinutesVisibleSpy
    let setDisplayTimeSpy
    let triggerSpy
    ...
    test('should call #setDisplayTime with 00 if isMilitaryFormat is true', () => {
      ...
      expect(setDisplayTimeSpy.calledWith({ hours: '00', minutes: '0' })).toBe(
        true,
      )
    })

    test('should call #setDisplayTime, index 0, with 12 if isMilitaryFormat is false', () => {
      ...
      expect(isMilitaryFormatStub.calledBefore(setDisplayTimeSpy)).toBe(true)
      expect(
        setDisplayTimeSpy.getCall(0).calledWith({ hours: '12', minutes: '0' }),
      ).toBe(true)
    })

    test('should set cachedEls.displayMeridiem.style.display to none when isMilitaryFormat is true', () => {
      isMilitaryFormatStub.onFirstCall().returns(true)
      picker.show()
    ...
    test('should set cachedEls.displayMeridiem.style.display to inline when isMilitaryFormat is false', () => {
      isMilitaryFormatStub.onFirstCall().returns(false)
      picker.show()
    ...
    test('should set cachedEls.meridiem.style.display to none when isMilitaryFormat is true', () => {
      isMilitaryFormatStub.onFirstCall().returns(true)
      picker.show()
    ...
    test('should set cachedEls.meridiem.style.display to block when isMilitaryFormat is false', () => {
      isMilitaryFormatStub.onFirstCall().returns(false)
      picker.show()
  ...
  describe('#hideEvent', () => {
    ...
    test('should call #hide if event.target.classList contains mtp-overlay', () => {
    ...
    test('should call #hide if event.target.classList contains mtp-actions__cancel', () => {
    ...
    test('should not call #hide if event.target.classList does not have allowed class', () => {
      picker.hideEvent(event)
  ...
  describe('#resetState', () => {
    ...
    let toggleMinutesVisibleSpy
    let isMilitaryFormatStub
    let hoursLiDispatchEventSpy
    let minutesLiDispatchEventSpy
    let militaryHoursLiDispatchEventSpy
    let meridiemSpanDispatchEventSpy

    beforeEach(() => {
      const { cachedEls } = picker
      ...
      toggleHoursVisibleSpy = sinon.spy(picker, 'toggleHoursVisible')
      toggleMinutesVisibleSpy = sinon.spy(picker, 'toggleMinutesVisible')
      hoursLiDispatchEventSpy = sinon.spy(
        cachedEls.clockHoursLi[0],
        'dispatchEvent',
      )
      minutesLiDispatchEventSpy = sinon.spy(
        cachedEls.clockMinutesLi[0],
        'dispatchEvent',
      )
      militaryHoursLiDispatchEventSpy = sinon.spy(
        cachedEls.clockMilitaryHoursLi[0],
        'dispatchEvent',
      )
      meridiemSpanDispatchEventSpy = sinon.spy(
        cachedEls.meridiemSpans[0],
        'dispatchEvent',
      )
    })
    ...
    test('should call #toggleHoursVisible with parameters true, and result of #isMilitaryFormat', () => {
    ...
    test('should call dispatchEvent with click event on cachedEls.clockHoursLi[0]', () => {
      picker.resetState()
    ...
    test('should call dispatchEvent with click event on cachedEls.clockMinutesLi[0]', () => {
      picker.resetState()
    ...
    test('should call dispatchEvent with click event on cachedEls.clockMilitaryHoursLi[0]', () => {
      picker.resetState()
    ...
    test('should call dispatchEvent with click event on cachedEls.meridiemSpans[0]', () => {
      picker.resetState()
  ...
  describe('#rotateHand', () => {
    test('should set cachedEls.clockHand.style.transform to rotate((nodeIndex * 30 - 180)deg)', () => {
      const nodeIndex = 5
      const rotateVal = 5 * 30 - 180

      picker.rotateHand(nodeIndex)

      expect(picker.cachedEls.clockHand.style.transform).toBe(
  ...
  describe.skip('#changeStep', () => {
    ...
    let toggleHoursVisibleSpy
    let toggleMinutesVisibleSpy
    let getActiveIndexStub
    let rotateHandSpy
    let timeSelectedStub
    let hideSpy
    ...
    test('should call #toggleHoursVisible(true) when step parameter is 0', () => {
      isMilitaryFormatStub.onFirstCall().returns(false)
    ...
    test(`should call #getActiveIndex of cachedEls.clockHoursLi
      ...
      expect(getActiveIndexStub.calledWith(picker.cachedEls.clockHoursLi)).toBe(
        true,
      )
    ...
    test(`should call #getActiveIndex with cachedEls.clockMilitaryHoursLi
      ...
      expect(getActiveIndexStub.calledOnce).toBe(true)
      expect(
        getActiveIndexStub.calledWith(picker.cachedEls.clockMilitaryHoursLi),
      ).toBe(true)
      expect(rotateHandSpy.calledOnce).toBe(true)
      expect(rotateHandSpy.calledWith(1)).toBe(true)
    })

    test('should call #toggleMinutesVisible(true) when step parameter is 1', () => {
      getActiveIndexStub.onFirstCall().returns(1)
    ...
    test('should call #getActiveIndex with cachedEls.clockMinutesLi when step parameter is 1', () => {
      getActiveIndexStub.onFirstCall().returns(1)
      ...
      expect(getActiveIndexStub.calledOnce).toBe(true)
      expect(
        getActiveIndexStub.calledWith(picker.cachedEls.clockMinutesLi),
      ).toBe(true)
    })

    test('should call #timeSelected and #hide when step parameter is 2', () => {
      picker.changeStep(2)
  ...
  describe('#toggleHoursVisible', () => {
    let clockHours
    let clockMilitaryHours

    beforeEach(() => {
      ;({ clockHours, clockMilitaryHours } = picker.cachedEls)
    })
  ...
  describe('#toggleMinutesVisible', () => {
    ...
    beforeEach(() => {
      ;({ clockMinutes, buttonBack } = picker.cachedEls)
    })

    test('should set cachedEls.clockMinutes.style.display to block when isVisible is true', () => {
      picker.toggleMinutesVisible(true)

      expect(clockMinutes.style.display).toBe('block')
    })

    test('should set cachedEls.clockMinutes.style.display to none when isVisible is false', () => {
      picker.toggleMinutesVisible(false)

      expect(clockMinutes.style.display).toBe('none')
    })

    test('should set cachedEls.buttonBack.style.display to inline-block when isVisible is true', () => {
      picker.toggleMinutesVisible(true)

      expect(buttonBack.style.display).toBe('inline-block')
    })

    test('should set cachedEls.buttonBack.style.display to none when isVisible is false', () => {
      picker.toggleMinutesVisible(false)
  ...
  describe('#getActiveIndex', () => {
    test('should return the index of element with .mtp-clock--active', () => {
      ...
      const { clockHoursLi } = picker.cachedEls
  ...
  describe('#setActiveEl', () => {
    test('should remove class .mtp-clock--active from element in containerEl parameter', () => {
      const { clockHoursLi } = picker.cachedEls
  ...
  describe('#meridiemSelectEvent', () => {
    test('should find child element in cachedEls.meridiem with class .mtp-clock--active and remove class', () => {
    ...
    test('should add class .mtp-clock--active to event.target element', () => {
      const { meridiemSpans } = picker.cachedEls
    ...
    test('should set cachedEls.displayMeridiem.innerHTML to event.target.innerHTML', () => {
      const { meridiemSpans } = picker.cachedEls
      const event = { target: meridiemSpans[1] }
  ...
  describe('#hourSelectEvent', () => {
    ...
    test('should call setActiveEl with containerEl and event.target parameters', () => {
      picker.hourSelectEvent(event, containerEl, listEls)
    ...
    test('should call setDisplayTime with event.target.innerHTML and 0 as index parameter', () => {
      picker.hourSelectEvent(event, containerEl, listEls)

      expect(setDisplayTimeSpy.calledOnce).toBe(true)
      expect(
        setDisplayTimeSpy.calledWith({ hours: event.target.innerHTML }),
      ).toBe(true)
    })
  ...
  describe('#minuteSelectEvent', () => {
    ...
    test('should call setDisplayTime with calculated value and 1 as index parameter', () => {
      picker.minuteSelectEvent(event, containerEl, listEls)

      expect(setDisplayTimeSpy.calledOnce).toBe(true)
      expect(
        setDisplayTimeSpy.calledWith({ minutes: sinon.match.number }),
      ).toBe(true)
    })
